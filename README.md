...
发布前须在仓库 Actions secrets 配置 `ANDROID_KEY_BASE64`（JKS 文件的 base64）、`ANDROID_KEY_ALIAS`、`ANDROID_KEY_PASSWORD`。签名密钥和密码不可提交到仓库；后续版本必须复用同一签名密钥，否则现有安装无法直接升级。SSH 远程和首个提交已推送；签名密钥尚待配置，发布工作流尚未运行。
